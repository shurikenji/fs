...
log "Deploy proxy-gateway từ ref $RESOLVED_REF"
extract_release_subtree "$RESOLVED_REF" "$RELEASE_DIR"
prepare_proxy_gateway_release "$RELEASE_DIR"
...
if ! restart_app_runtime || ! run_runtime_smoke_checks; then
  ...
  fail "Deploy proxy-gateway thất bại"
fi

log "Deploy proxy-gateway thành công: $RELEASE_DIR"
